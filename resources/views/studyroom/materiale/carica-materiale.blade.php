...
<section class="upload-card">
    ...
    <form method="POST" enctype="multipart/form-data" action="{{ route('studyroom.materiali.salva') }}" class="upload-form">
        ...
        <x-combo-box 
            ...
            :value="$selectedInsNome ?? ''" :hiddenValue="$selectedIns ?? ''" required disabled>
        ...
        <div class="form-group">
            ...
            @error('titolo') <div class="error-msg">{{ $message }}</div> @enderror
        </div>

        <!-- TAG -->
        <div class="form-group" id="tagGroup">
            <label for="tagSelect">Tag</label>
            <select name="tag" id="tagSelect" {{ old('tipo', $tipo ?? 'appunto') == 'esame' ? 'disabled' : '' }}>
            ...
            @error('tag') <div class="error-msg">{{ $message }}</div> @enderror
        </div>
